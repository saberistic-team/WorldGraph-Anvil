...
const canonicalMigrationRoot = resolve('packages/db/drizzle');
const databaseOrigin =
  process.env['WORLDGRAPH_M09_DATABASE_ORIGIN'] === 'upgrade' ? 'upgrade' : 'clean';
const testLogLevel = process.env['WORLDGRAPH_TEST_LOG_LEVEL'] === 'error' ? 'error' : 'fatal';
const m08MigrationTags = [
...
interface CommerceScheduleRepositoryModule {
  PostgresCommerceScheduleRepository: new (
    pool: Pool,
...
interface M08WorldSnapshot {
  columns: Record<string, string[]>;
  documents: Record<string, unknown>;
  rows: Record<string, unknown>;
}

const m08DirectWorldTables = [
...
const m08WorldSnapshotSpecs: readonly WorldSnapshotSpec[] = [
  {
    alias: 'source_row',
    from: 'worlds source_row',
    table: 'worlds',
    where: 'source_row.id = $1',
  },
  ...m08DirectWorldTables.map((table) => ({
    alias: 'source_row',
    from: `${table} source_row`,
    table,
    where: 'source_row.world_id = $1',
  })),
  {
    alias: 'source_row',
    from: `manifest_validation_reports source_row
      join manifest_revisions parent on parent.id = source_row.manifest_revision_id`,
    table: 'manifest_validation_reports',
    where: 'parent.world_id = $1',
  },
  {
    alias: 'source_row',
    from: `manifest_field_provenance source_row
      join manifest_revisions parent on parent.id = source_row.manifest_revision_id`,
    table: 'manifest_field_provenance',
    where: 'parent.world_id = $1',
  },
  {
    alias: 'source_row',
    from: `manifest_provider_calls source_row
      join manifest_generation_runs parent on parent.id = source_row.run_id`,
    table: 'manifest_provider_calls',
    where: 'parent.world_id = $1',
  },
  {
    alias: 'source_row',
    from: `generation_retrieval_items source_row
      join manifest_generation_runs parent on parent.id = source_row.run_id`,
    table: 'generation_retrieval_items',
    where: 'parent.world_id = $1',
  },
  {
    alias: 'source_row',
    from: `event_consumer_receipts source_row
      join domain_events parent on parent.id = source_row.event_id`,
    table: 'event_consumer_receipts',
    where: 'parent.world_id = $1',
  },
  {
    alias: 'source_row',
    from: `economy_reconciliation_items source_row
      join economy_reconciliation_runs parent on parent.id = source_row.run_id`,
    table: 'economy_reconciliation_items',
    where: 'parent.world_id = $1',
  },
];

const m09DirectWorldTables = [
  'business_facilities',
  ...
  'payroll_policy_selection_facts',
  'payroll_records',
...
async function createM08MigrationRoot(): Promise<string> {
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'worldgraph-m08-upgrade-migrations-'));
  await mkdir(join(temporaryRoot, 'meta'));
  await Promise.all(
    m08MigrationTags.map((tag) =>
      cp(join(canonicalMigrationRoot, `${tag}.sql`), join(temporaryRoot, `${tag}.sql`)),
    ),
  );
  await writeFile(
    ...
    JSON.stringify({
      version: '7',
      dialect: 'postgresql',
      entries: m08MigrationTags.map((tag, idx) => ({
        idx,
        version: '7',
...
async function captureM08WorldSnapshot(
  pool: Pool,
  worldId: string,
  retainedColumns?: Readonly<Record<string, readonly string[]>>,
): Promise<M08WorldSnapshot> {
  const tableNames = m08WorldSnapshotSpecs.map((spec) => spec.table);
  let columns: Record<string, string[]>;
  if (retainedColumns) {
    columns = Object.fromEntries(
      Object.entries(retainedColumns).map(([table, names]) => [table, [...names]]),
    );
  } else {
  ...
  for (const spec of m08WorldSnapshotSpecs) {
    const selectedColumns = columns[spec.table];
    if (!selectedColumns || selectedColumns.length === 0) {
      throw new Error(`M08 snapshot table is unavailable: ${spec.table}`);
    }
    const selectList = selectedColumns
      .map((column) => `${spec.alias}.${sqlIdentifier(column)}`)
      .join(',');
    const result = await pool.query<{ value: unknown }>(
      `select coalesce(
         jsonb_agg(to_jsonb(snapshot_row) order by to_jsonb(snapshot_row)::text collate "C"),
         '[]'::jsonb
       ) as value
       from (
         select ${selectList}
    ...
    rows[spec.table] = result.rows[0]!.value;
  }

  const documentResult = await pool.query<Record<string, unknown>>(
    `select
       worldgraph_projection_document($1) as graph,
       worldgraph_simulation_projection_document($1) as simulation,
       worldgraph_economy_projection_document($1) as economy,
       worldgraph_materialized_economy_seed_plan($1) as materialized_plan,
       worldgraph_economy_reconciliation_snapshot($1) as reconciliation`,
    [worldId],
  );
  return { columns, documents: documentResult.rows[0]!, rows };
}

async function assertActiveM08World(pool: Pool, worldId: string): Promise<void> {
  const identity = await pool.query<{
    artifact_count: number;
    artifact_schema_version: number;
    compiled_controllers: number;
    compiled_entities: number;
    compiled_relationships: number;
    ...
    `select
       ...
       plan.seed_plan_schema_version as plan_schema_version,
       plan.source_kind::text,plan.source_compiler_version,
       plan.source_adapter_id,plan.source_adapter_version,
       (select count(*)::integer from compiled_world_artifacts stored
         where stored.world_id=$1) as artifact_count,
       jsonb_array_length(artifact.canonical_content -> 'entities') as compiled_entities,
       jsonb_array_length(artifact.canonical_content -> 'relationships') as compiled_relationships,
       jsonb_array_length(artifact.canonical_content -> 'controllers') as compiled_controllers,
       (select count(*)::integer from world_entities entity
         where entity.world_id=$1 and entity.retired_world_version_id is null) as entity_count,
       ...
       (select count(*)::integer from world_entity_controllers controller
         where controller.world_id=$1 and controller.revoked_at is null) as controller_count
     ...
     join compiled_economy_seed_plans plan
       on plan.world_id=version.world_id and plan.world_version_id=version.id
    where version.world_id=$1 and version.status='active'`,
    [worldId],
  );
  ...
  expect(row).toMatchObject({
    ...
    source_adapter_id: 'CompiledEconomySeedAdapterV1',
    source_adapter_version: '1.0.0',
    source_compiler_version: '1.1.0',
    source_kind: 'compiler_1_1',
    version_compiler_version: '1.1.0',
  });
  expect(row.entity_count).toBe(row.compiled_entities);
  expect(row.relationship_count).toBe(row.compiled_relationships);
  expect(row.controller_count).toBe(row.compiled_controllers);

  const ids = await pool.query<{
    compilation_run_id: string;
    economy_command_id: string;
  }>(
    `select version.compilation_run_id::text,
            economy.initialized_command_id::text as economy_command_id
       from world_versions version
       join world_economy_heads economy on economy.world_id=version.world_id
      where version.world_id=$1 and version.status='active'`,
    [worldId],
  );
  expect(ids.rows).toHaveLength(1);
  const authority = ids.rows[0]!;
  await pool.query(
    `select
       worldgraph_assert_compilation_run_consistency($1),
       worldgraph_assert_active_world_graph($2),
       worldgraph_assert_active_world_ledger($2),
       worldgraph_assert_active_world_simulation($2),
       worldgraph_assert_simulation_projection_current($2),
       worldgraph_assert_command_terminal($3),
       worldgraph_assert_economy_command_terminal($3),
       worldgraph_assert_economy_projection_current($2)`,
    [authority.compilation_run_id, worldId, authority.economy_command_id],
  );
  const economy = await pool.query<{
    exact_mutations: boolean;
    event_hashes_valid: boolean;
    head_hash_valid: boolean;
    ledger_chain_valid: boolean;
    ledger_hashes_valid: boolean;
    materialized_plan_valid: boolean;
    matched: boolean;
    mismatch_count: number;
  }>(
    `with ordered_ledger as (
       select entry.*,
              lag(entry.entry_hash) over (order by entry.ledger_sequence) as prior_entry_hash,
              row_number() over (order by entry.ledger_sequence) as ordinal
         from ledger_entries entry where entry.world_id=$1
     ), reconciliation as (
       select worldgraph_economy_reconciliation_snapshot($1) value
     )
     select
       worldgraph_economy_command_mutation_set_is_exact($2) as exact_mutations,
       worldgraph_materialized_economy_seed_plan($1) = plan.canonical_plan
         as materialized_plan_valid,
       (reconciliation.value ->> 'matched')::boolean as matched,
       (reconciliation.value ->> 'mismatchCount')::integer as mismatch_count,
       not exists (
         select 1 from domain_events event where event.world_id=$1 and event.event_hash <>
       ...
       ) as event_hashes_valid,
       not exists (
         select 1 from ordered_ledger entry where entry.entry_hash <>
       ...
       ) as ledger_hashes_valid,
       not exists (
         select 1 from ordered_ledger entry
          where entry.previous_hash <> case when entry.ordinal=1
            then decode(repeat('00',32),'hex') else entry.prior_entry_hash end
       ) as ledger_chain_valid,
       head.last_entry_hash = (
         select entry.entry_hash from ordered_ledger entry order by entry.ledger_sequence desc limit 1
       ) as head_hash_valid
     from compiled_economy_seed_plans plan
     join world_ledger_heads head on head.world_id=plan.world_id
     cross join reconciliation
     where plan.world_id=$1`,
    [worldId, authority.economy_command_id],
  );
  expect(economy.rows).toEqual([
    {
      exact_mutations: true,
      event_hashes_valid: true,
      head_hash_valid: true,
      ledger_chain_valid: true,
      ledger_hashes_valid: true,
      materialized_plan_valid: true,
      matched: true,
      mismatch_count: 0,
    },
  ]);
...
async function assertFreshM08Simulation(pool: Pool, worldId: string): Promise<void> {
  const result = await pool.query<{
    ...
    `select command.id::text as command_id,runtime.state_revision::text,
       ...
       join command_records command
         on command.world_id=runtime.world_id
  ...
  expect(result.rows[0]!.command_id).toMatch(/^[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}$/iu);
  await pool.query(
    `select worldgraph_assert_active_world_graph($1),
            worldgraph_assert_active_world_ledger($1),
            worldgraph_assert_active_world_simulation($1),
            worldgraph_assert_simulation_projection_current($1),
            worldgraph_assert_simulation_command_terminal($2)`,
    [worldId, result.rows[0]!.command_id],
  );
}

async function assertNoM09State(pool: Pool, worldId: string): Promise<void> {
  const directCounts = m09DirectWorldTables
    .map((table) => `select count(*)::bigint as count from ${table} where world_id=$1`)
    .join(' union all ');
  const result = await pool.query<{ count: string }>(
    `select coalesce(sum(source.count),0)::text as count from (
       ${directCounts}
       union all
       select count(*)::bigint
         from economy_expansion_reconciliation_items item
         join economy_expansion_reconciliation_runs run on run.id=item.run_id
  ...
  await expect(
    pool.query(
      `select 1 from projection_checkpoints
        where world_id=$1 and projection_name='economy_closed_loop'`,
      [worldId],
    ),
  ...
  await expect(
    pool.query(
      `select count(*)::integer as count from command_records
        where world_id=$1 and rate_limit_scope_hash is not null`,
      [worldId],
    ),
...
describe.sequential(
  `M08/M09 economy commands with real PostgreSQL from a ${databaseOrigin} database`,
  () => {
    ...
    let temporaryM08MigrationRoot: string | undefined;
    ...
    beforeAll(async () => {
      ...
      client.pool.options.query_timeout = 60_000;
      client.pool.options.statement_timeout = 60_000;
      if (databaseOrigin === 'upgrade') {
        temporaryM08MigrationRoot = await createM08MigrationRoot();
        await applyMigrations(client, temporaryM08MigrationRoot);
        ...
        await importStarterPrimitives(client.pool, { includeHarborCityEconomy: false });
      } else {
      ...
      appClient.pool.options.query_timeout = 60_000;
      appClient.pool.options.statement_timeout = 60_000;
      ...
      if (databaseOrigin === 'upgrade') {
        const preservedWorld = await createCompiledWorld(
          'M08 Active Upgrade Preservation',
          'm08-upgrade-preservation',
        );
        await assertFreshM08Simulation(client.pool, preservedWorld.worldId);
        await initialize(preservedWorld);
        await assertActiveM08World(client.pool, preservedWorld.worldId);
        const beforeUpgrade = await captureM08WorldSnapshot(client.pool, preservedWorld.worldId);
        ...
        await assertActiveM08World(client.pool, preservedWorld.worldId);
        const afterUpgrade = await captureM08WorldSnapshot(
          client.pool,
          preservedWorld.worldId,
          beforeUpgrade.columns,
        );
        expect(afterUpgrade).toEqual(beforeUpgrade);
        await assertNoM09State(client.pool, preservedWorld.worldId);
    ...
    afterAll(async () => {
      ...
      if (temporaryM08MigrationRoot) {
        await rm(temporaryM08MigrationRoot, { force: true, recursive: true });
      }
    });
    ...
    it('initializes exactly, enforces participant privacy, and serializes 100 overspends plus opposite transfers', async () => {
      ...
      const acceptedReplay = await submit(
        creator,
        stressWorld.worldId,
        acceptedOverspends[0]!.body,
      );
    ...
    it('runs Harbor commerce with access boundaries, terminal races, atomic rollback, and reconciliation end to end', async () => {
      ...
      expect(await runCommerceScheduleOnce(appClient.pool, ids, commerceWorld.worldId)).toEqual([]);
      ...
      expect(raceBuyerState.rows.every((buyer) => BigInt(buyer.available_minor) >= 102n)).toBe(
        true,
      );
      ...
      const expensiveListed = await submit(
        manager!,
        commerceWorld.worldId,
        expensiveListingCommand,
      );
      ...
      const beforeRollback = await commerceAtomicFootprint(
        commerceWorld.worldId,
        rollbackListingId,
      );
      ...
      const expectedTreasuryRevenue = terminalRaceState.status === 'filled' ? '31' : '29';
      expect(treasuryRead.json()).toMatchObject({
        treasury: { revenueMinor: expectedTreasuryRevenue },
      });
      await expect(
        client.pool.query(
          `select coalesce(sum(amount_minor),0)::text as revenue_minor
           from tax_assessments where world_id=$1`,
          [commerceWorld.worldId],
        ),
      ).resolves.toMatchObject({ rows: [{ revenue_minor: expectedTreasuryRevenue }] });
    ...
    it('settles taxed payroll exactly once across duplicate delivery, response loss, and rollback retry', async () => {
      ...
      const createContract = commerceCommandFromContext(
        ...
        'CreateEmploymentContractV1',
        {
          ...
          periodTicks: '12',
          rewardCapMinor: '300',
          roleCode: 'payroll_recovery_tester',
          wageMinor: '100',
          wageRuleKind: 'per_shift',
          workerEntityKey: workerPerson!.entity_key,
          workerWalletId: workerPerson!.wallet_id,
      ...
      expect(await runCommerceScheduleOnce(appClient.pool, ids, world.worldId)).toEqual([]);
    }, 240_000);
    ...
    it('replays twin Harbor catch-up schedules to identical ledger and projection evidence', async () => {
      ...
      expect(replay.eventEvidence).toEqual(first.eventEvidence);
      ...
      expect(replay.projectionEvidence).toEqual(first.projectionEvidence);
      ...
      // Compiled principals, player logical keys, and their seed wallets are intentionally
      // salted per world to prevent cross-world user correlation; semantic evidence above
      // must replay exactly while the two raw provenance identities remain world-scoped.
      expect(replay.sourceArtifactHash).not.toBe(first.sourceArtifactHash);
      expect(replay.seedPlanHash).not.toBe(first.seedPlanHash);
    ...
    it('repairs a journal-proven Harbor inventory mismatch through independent append-only approval', async () => {
      ...
      try {
        ...
        await corruptionConnection.query(
          ...
          [commerceWorld.worldId, inventory.id],
        );
      ...
      const evidence = await client.pool.query<{
        ...
        inventory_reserved_quantity: string;
        ledger_kind: string;
        reconciliation_schema_version: number;
        reconciliation_status: string;
        ...
        `select
         ...
         (select count(*)::integer from domain_events
           where command_id=$7) as repair_event_count,
         ...
         reconciliation.reconciliation_schema_version,
      ...
      const v3Snapshot = await client.pool.query<{
        snapshot: { itemCount: number; matched: boolean; mismatchCount: number };
      }>(`select worldgraph_reconcile_economy_expansion_v3($1,null::uuid) as snapshot`, [
        commerceWorld.worldId,
      ]);
      expect(v3Snapshot.rows[0]!.snapshot).toMatchObject({
        itemCount: 0,
        matched: true,
        mismatchCount: 0,
      });
    ...
    it('detects every mutable M09 projection category and rejects reservation-record repair', async () => {
      ...
      const corruptionCases = [
        ...
        {
          expectedKind: 'production',
          mutation: `with target as (
            select id,
                   jsonb_set(
                     input_snapshot,'{0,quantity}',
                     to_jsonb('999999.000000000000'::text),false
                   ) as changed_input_snapshot,
                   output_snapshot
          ...
          update production_runs run
             set input_snapshot=target.changed_input_snapshot,
                 snapshot_checksum=extensions.digest(convert_to(
                   worldgraph_canonical_jsonb(jsonb_build_object(
                     'inputs',target.changed_input_snapshot,
                     'outputs',target.output_snapshot
                   )),'UTF8'
                 ),'sha256')
            from target
        ...
        },
        {
          ...
          mutation: `with target as (
            select id,input_snapshot,
                   jsonb_set(
                     output_snapshot,'{0,quantity}',
                     to_jsonb('999999.000000000000'::text),false
                   ) as changed_output_snapshot
              from production_runs
             where world_id=$1
             order by id
             limit 1
          ...
          update production_runs run
             set output_snapshot=target.changed_output_snapshot,
                 snapshot_checksum=extensions.digest(convert_to(
                   worldgraph_canonical_jsonb(jsonb_build_object(
                     'inputs',target.input_snapshot,
                     'outputs',target.changed_output_snapshot
                   )),'UTF8'
                 ),'sha256')
            from target
           where run.id=target.id`,
    ...
    async function advanceCommerceTo(
      ...
      const results = await runCommerceScheduleOnce(appClient.pool, ids, worldId);
      expect(
    ...
    async function runDeterministicHarborSchedule(
      ...
      expect(
        catchUpResults.every((result) => result.outcome === 'applied'),
        JSON.stringify(catchUpResults),
      ).toBe(true);
      ...
      const [
        ...
        client.pool.query<SeedRow>(
          `select case when controller.user_id is null then wallet.stable_key::text
            ...
            order by wallet_key,balance.available_minor,balance.row_version`,
          [worldId],
        ),
        client.pool.query<SeedRow>(
    ...
    async function deterministicHarborEvidence(
      ...
      return {
        ...
        eventEvidence: canonicalReplayRows(events.rows),
        eventHashIntegrity,
        ...
        projectionEvidence: projectionDocument,
        seedChecksum,
      ...
      const normalize = (value: unknown): unknown => {
        if (typeof value === 'string') {
          return value
            ...
            .replace(digestPattern, (digest) => {
              const normalized = digest.toLowerCase();
              let alias = digests.get(normalized);
              if (!alias) {
                alias = `digest:${nextDigest.toString()}`;
                nextDigest += 1;
                digests.set(normalized, alias);
              }
              return alias;
            });
        }
...
async function runCommerceScheduleOnce(
  ...
  const runner = new workerModule.CommerceScheduleRunner(
    new repositoryModule.PostgresCommerceScheduleRepository(pool, [], worldIdScope),
    scheduledCommands,
    createLogger({
      buildRevision: 'test',
      environment: 'test',
