...
Status: **complete and sealed on August 3, 2026 EDT**. The implementation, migration, regression, exact-upgrade, two-user browser, clean/retained Compose recovery, and supply-chain gates recorded below passed. This record authorizes Milestone 10 with the cumulative compatibility state, architecture decisions, and retained risks below as mandatory inputs.
...
The web Economy workspace exposes resource, business, production, market, treasury, tax, and reconciliation views with a no-real-money disclosure. It provides business creation, facility configuration, employment creation/acceptance/end, work performance, production start, listing creation/cancellation, and confirmed purchase controls. A zero-lag pending reconciliation permits actions while visibly reporting `Reconciliation pending`; nonzero lag, a mismatch, or a failed reconciliation blocks mutations. The isolated desktop/mobile/axe suite passed 82/82, and the real two-context manager/buyer browser journey passed on both clean-volume and retained-data Compose stacks.
...
The real PostgreSQL suites execute fresh and exact-upgrade cases for all five payload-fact histories, ended employment, completed and failed production, a filled listing/trade, authority hashes, ambiguous-listing rejection, transactional rollback, rate/abuse controls, concurrency, and outbox-retry ownership/constraints. The full integration gate passed 22 files/228 tests. A separate production-shaped upgrade-origin gate passed 1 file/12 tests, preserving a genuinely active sealed-M08 world without synthesized M09 rows before compiling and running the native plan-2 harbor demo on that same upgraded database. Clean bootstrap, retained-data upgrade, repeat application, and rollback behavior are executed evidence rather than inferred results.
...
PostgreSQL pool failure handling observes both pg-pool's idle-client error path and every checked-out client between pool `acquire` and `release`. This closes the Node.js unhandled-`error` crash exposed when PostgreSQL was stopped under retained multi-world worker load, without changing repository release ownership. API and worker callbacks emit only bounded stable codes and messages; observer failure is contained rather than rethrowing the original database error. The definitive clean and retained recovery drills stopped and restarted PostgreSQL while proving that the same API and worker processes remained alive and returned to healthy readiness.
...
Final acceptance ran through August 3, 2026 EDT with Node.js 24.18 and pnpm 11.9. Every result below comes from the sealed tree; earlier partial or interrupted attempts are retained only in the correction record and are not counted independently.

| Check                                             | Sealed result                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Aggregate repository gate                         | **PASS.** `pnpm check` completed format, lint, 16 package typechecks and production builds, migration journal validation at head `0012`, current/previous/legacy goldens, standalone compiler CLI, and 111 unit files/734 tests.                                                                                                                                                                                                                                 |
| Real PostgreSQL/Redis integration                 | **PASS.** The full suite completed 22 files/228 tests, including transactional, concurrency, recovery, authority, privacy, rate/abuse, migration, and operator scenarios.                                                                                                                                                                                                                                                                                        |
| Exact sealed-M08 upgrade origin                   | **PASS.** The dedicated production-shaped suite completed 1 file/12 tests: it preserved the active compiler-1.1/artifact-2/plan-1 world and all M08 authority, synthesized no M09 state, then ran the compiler-1.2/artifact-3/plan-2 harbor journey on the same migrated database.                                                                                                                                                                               |
| Browser/accessibility                             | **PASS.** Playwright completed 82/82 desktop/mobile cases, including keyboard, responsive, and axe coverage.                                                                                                                                                                                                                                                                                                                                                     |
| Clean plus retained-data Compose browser/recovery | **PASS.** The definitive current images completed the full two-session manager/buyer browser commerce journey first from clean volumes and then against retained data. Both runs exercised Redis and PostgreSQL loss/recovery; the API and worker remained the same live processes and returned to healthy readiness.                                                                                                                                            |
| Migration journal/checksum                        | **PASS.** Head `0012`, index 11, with `0010` `6cb7ba473224fc08ad306771db8096aa4f649c28910619144c971f05d91524e4`, `0011` `9b1923c5ccabe62abb1502f7a525f0052bd7c3e015b0ec41aa1bb7833fa7c3b6`, and `0012` `f46f1c39b0e5e8365a7175c96fc72d72e7e0243d7d6e1a010aa31f1aa539aced`.                                                                                                                                                                                       |
| Dependency and license review                     | **PASS with retained low-severity development advisories.** The high gate found zero high/critical advisories; the 241-package production graph found zero advisories at every severity. The complete graph retains development-only esbuild advisories at `0.18.20` (one moderate) and `0.27.7` (one low). The clean license inventory found 11 license groups, no missing license and no GPL-3.0/AGPL-3.0 package; LGPL and CC attribution obligations remain. |
| Image review                                      | **PASS for the reviewed local properties, without a container-CVE-scanner claim.** API, migrate, worker, and web run as non-root `node` UID/GID 1000 in `/app`; each reports the same 46 rootfs layers. Separate local attestation manifest identifiers begin `api:60f40e`, `migrate:6528aa`, `worker:b642ef`, and `web:3abb7c`. Base and CI image references remain mutable tags, so this is neither immutable digest pinning nor a CVE scan.                   |
| Independent implementation audits                 | **PASS.** Independent reviews found no remaining P0/P1 defect. The replay-authority, privilege-boundary, retry-idempotency, tax-scope, PostgreSQL client-error, and acceptance-fixture findings were corrected and reverified.                                                                                                                                                                                                                                   |

### Retry and correction record

- The earlier dependency restoration interruption and 717/718 partial unit run are historical diagnostics, not final evidence. After dependencies were restored, the compiler contention limits were made explicit and the complete current tree passed 111 files/734 tests under `pnpm check`.
- The deterministic `commerce-invariants` coverage still executes 500 generated sequences checking money conservation, balanced postings, inventory/reservation bounds, recipe output, payroll uniqueness, and ownership. The sealed integration and Compose gates now supply the complementary live PostgreSQL/Redis evidence.
- Independent SQL review found that terminal production authority could otherwise be self-authenticated from a corrupted version. The migration binds exact completed/failed versions, selected events, full terminal snapshots, event ordinals/ticks, the immutable initial transition, and recipe/tax-policy provenance in aggregate hashes.
- Independent outbox review found an application-role privilege path into the private retry table and a stale replay semantic. Narrow security-definer lookup, explicit revocation, forged-gate tests, stable replay fields, refusal to reuse a retry identity after a later death, and a final tuple-exactness CLI check closed those issues.
- Payroll duplicate/recovery, failed-production reconciliation, purchase replay corruption, migration upgrade, abuse-signal, identical-scope tax-policy, and the 50-independent-buyer final-unit scenarios all ran in the sealed live database suites.
- A proposed direct-insert active-M08 migration fixture was rejected because it was not runtime-producible. The replacement exact-origin fixture builds a production-shaped active sealed-M08 world, proves unchanged authority after upgrade, and performs the native plan-2 demo on that same database; its dedicated gate passed 12/12.
- Retained-data outage testing exposed an unhandled Node.js `Client` error while a PostgreSQL pool client was checked out. The shared database pool now installs lifecycle-aware acquire/release observers in addition to the idle-client observer, contains observer failures, and gives API/worker only bounded code-level logging. Clean and retained drills then proved PostgreSQL recovery without replacing either process.
...
- Database error observation is a shared pool concern: pg-pool's idle error listener is complemented only while a client is checked out, using pool acquire/release lifecycle events. This gives API and worker the same crash boundary without changing transaction ownership or adding repository-specific listeners.
...
- Executed abuse coverage rejects a same-entity self-purchase with a bounded `self_trade_attempt` signal, emits the same signal when one actor controls distinct buyer and seller entities, and detects the bounded A→B→C→A `rapid_circular_transfer` pattern.
...
- Operator replay can leave an inert `pending` or `running` evidence row if PostgreSQL disappears after authorization but before finalization. That row cannot authorize repair, block a new replay, or create a partial/authoritative shadow; a future milestone should harden the finalizer so this operational evidence terminalizes automatically after recovery.
...
- Dashboard/rule deployment, target-environment OTLP translation, threshold calibration, alert routing, receiver configuration, and a completed drill remain deployment risks even though their checked-in M09 definitions exist.
- The inherited shared application database role, managed PITR/restore drills, external immutable checkpoints, continuous privileged scans, alert routing, provider/profile hardening, pruned service images, and managed deployment risks remain.
- Container base and CI actions use mutable version tags rather than immutable digests. The local image inspection is not a container vulnerability scan, and the runtime images retain a broader development dependency tree than a pruned compiled bootstrap would require.
- The full dependency graph retains development-only esbuild `0.18.20` at moderate severity and `0.27.7` at low severity. LGPL and CC notices/dynamic-link attribution remain release obligations even though the clean inventory found no missing, GPL-3.0, or AGPL-3.0 license.
...
## Inputs for Milestone 10

**Milestone 10 is authorized.** It must consume API v1; contract/runtime 9; compiler `1.2.0`/artifact 3/seed plan 2 plus exact retained compiler-1.1/artifact-2/plan-1 and compiler-1.0/artifact-1 lanes; core-economy schema 1; resource/business/facility/employment/production/market/trade/tax/commerce schema 1; economy reconciliation 2 with schema 1 retained; simulation process registry 2 with unchanged action/process/PRNG schemas; journal head `0012` and all exact migration hashes; ADR 0014; every command/event/route, scheduler, reconciliation-authority, repair, outbox-retry, observability, rate-limit, abuse-signal, database-pool, and no-real-money decision above; the final verification evidence; and every retained risk. M10 must preserve prior bytes and cannot reinterpret the seal as authorization for external money, client economic authority, runtime AI decisions, or an unauthenticated browser realtime relay.
